...
###   bitcoinrandcore.org and bitcoinrand.org and compares them.
...
WORKINGDIR="/tmp/bitcoinrand_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://bitcoinrandcore.org"
HOST2="https://bitcoinrand.org"
BASEDIR="/bin/"
VERSIONPREFIX="bitcoinrand-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'bitcoinrand-',
...
if [ $? -ne 0 ]; then
   echo "bitcoinrand.org failed to provide signature file, but bitcoinrandcore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "bitcoinrand.org and bitcoinrandcore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Bitcoinrand Core binary release signing key installed?"
   fi
