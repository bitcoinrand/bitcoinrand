...
const char * const BITCOINRAND_CONF_FILENAME = "bitcoinrand.conf";
const char * const BITCOINRAND_PID_FILENAME = "bitcoinrandd.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "bitcoinrandd -foo=bar
...
#else
    const char* pszModule = "bitcoinrand";
#endif
    if (pex)
        return strprintf(
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Bitcoinrand
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Bitcoinrand
    // Mac: ~/Library/Application Support/Bitcoinrand
    // Unix: ~/.bitcoinrand
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Bitcoinrand";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Bitcoinrand";
#else
    // Unix
    return pathRet / ".bitcoinrand";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", BITCOINRAND_CONF_FILENAME);
...
fs::path GetPidFile()
{
    return AbsPathForConfigVal(fs::path(gArgs.GetArg("-pid", BITCOINRAND_PID_FILENAME)));
}
